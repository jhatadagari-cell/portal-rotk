...
const HacCombate = (function () {
  ...
  // ── Halo de fuego horneado una sola vez (nada de gradientes por frame) ────────────
  function bakeFx() {
  ...
  // ── Brasero: llama procedural por capas (aditiva) + halo pulsante. Parpadea y da un
  // leve vaivén propio (sin viento de escena). Coordenadas en px de dispositivo. ──────
  ...
  function drawBrazier(gx, gy, tt, seed) {
    ...
    const lean = Math.sin(tt * 0.013 + seed) * 0.14;   // vaivén suave de la propia llama
    ...
    if (Math.random() < 0.45) parts.push({ x: gx + rnd(-3, 3) * s, y: gy - 6 * s, vx: rnd(-0.15, 0.15) * s, vy: -rnd(0.35, 0.9) * s, life: 0, max: rnd(700, 1500), col: 'rgba(255,170,70,', r: rnd(1, 2.2) * s, ember: true });
  }
  ...
  function frame() {
    ...
    // braseros (llama parpadeante + halo)
