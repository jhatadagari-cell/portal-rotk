/* ═══════════════════════════════════════════════════════════════════════
   ...
   Reglas de paseo:
     · No atraviesa edificios ni murallas; SÍ cruza portones y puertas interiores.
     · Tiende a seguir los CAMINOS (×4).
     · Con edificio ASIGNADO (dueño): gravita a su alrededor y a veces se mete
       dentro (queda oculto = "entra"); su presencia se anuncia con el banner.

   API:
     HacFolk.start(iso, { mapa, tier, color, miembros, onState })  ·  stop()
     HacFolk.list()        → [{ id, name, color, inside: <label>|null }]
...
const HacFolk = (function () {
  ...
  // Mapa de la finca: celdas transitables, caminos, edificios de cada mecenas y
  // qué edificio cubre cada celda (para saber quién está DENTRO).
  function build(mapa, tier) {
    ...
    const buildings = new Map(), cellBuilding = new Map();
    lista.forEach(c => {
      ...
      // ¿Es un EDIFICIO en el que se puede "estar dentro"? (no suelo/muro/decoración)
      ...
      if (cat === 'edificio') {
        ...
        buildings.set(id, { id, cx, cy, altura: def.altura || 24, dueno: c.dueno || null, nombre: def.nombre || 'Edificio' });
        cells.forEach(p => cellBuilding.set(p[0] + ',' + p[1], id));
      }
    ...
    for (let y = 0; y < GH; y++) for (let x = 0; x < GW; x++) {
      const k = x + ',' + y; if (!blocked.has(k)) { set.add(k); cells.push([x, y]); }
    }
    return { set, cells, cam, GW, GH, ownByMember, buildings, cellBuilding };
  }
  ...
  function spawn(mapa, tier, miembros, color) {
    ...
    const camCells = wk.cells.filter(([x, y]) => wk.cam.has(x + ',' + y));
    return (miembros || []).slice(0, 24).map(m => {
      const own = wk.ownByMember[m.id] || [], ownCells = new Set(own);
      let home = null, start;
      if (own.length) {
        const mid = own[Math.floor(own.length / 2)].split(',').map(Number); home = mid;
        start = wk.cells.reduce((best, c) => { const d = Math.abs(c[0] - home[0]) + Math.abs(c[1] - home[1]); return d < best.d ? { c, d } : best; }, { c: wk.cells[0], d: 1e9 }).c;
      } else {
        start = (camCells.length && Math.random() < 0.7) ? camCells[rnd(camCells.length)] : wk.cells[rnd(wk.cells.length)];
      }
      return { id: m.id, name: m.nombre || '', color, fx: start[0], fy: start[1], tx: start[0], ty: start[1], moving: false, wait: Math.random() * 1.5, phase: Math.random() * 6.28, home, ownCells, inside: false };
    });
  }

  function pickTarget(w) {
    const cx = Math.round(w.fx), cy = Math.round(w.fy);
    const cand = neigh(cx, cy).filter(([x, y]) => wk.set.has(x + ',' + y) || w.ownCells.has(x + ',' + y));
    ...
    cand.forEach(([x, y]) => {
      ...
      if (w.home) {
        ...
        if (dNew < dNow) wgt += (dNow > 3 ? 6 : 3);
        if (w.ownCells.has(x + ',' + y)) wgt += 2;
      }
    ...
    w.enter = w.ownCells.has(t[0] + ',' + t[1]);
  }

  function step(dt) {
    const SPD = 1.1;
    walkers.forEach(w => {
      if (!w.moving) { w.wait -= dt; if (w.wait <= 0) pickTarget(w); }
      else {
        const dx = w.tx - w.fx, dy = w.ty - w.fy, d = Math.hypot(dx, dy), adv = SPD * dt;
        if (d <= adv) { w.fx = w.tx; w.fy = w.ty; w.moving = false; w.wait = (w.enter ? 1.4 : 0.3) + Math.random() * 1.8; }
        else { w.fx += dx / d * adv; w.fy += dy / d * adv; }
        w.phase += dt * 8;
      }
      // ¿En qué edificio está? (la celda actual cubierta por un edificio)
      w.insideId = wk.cellBuilding.get(Math.round(w.fx) + ',' + Math.round(w.fy)) || null;
    });
  }
  ...
  function drawWalker(g, lx, ly, w, o) {
    ...
    const bob = Math.sin(w.phase) * (w.moving ? 1.2 : 0.3);
  ...
  function paint() {
    ...
    // Agrupa quién está DENTRO de cada edificio.
  ...
  // Avisa a la página cuando cambia el "estado social" (quién está dentro de qué,
  // o el seleccionado) para que refresque el listado lateral.
  function pushState() {
    const sig = selectedId + '|' + walkers.map(w => w.id + ':' + (w.insideId || '')).join(',');
  ...
  function start(isoCanvas, o) {
    ...
    walkers.forEach(w => { w.insideId = wk.cellBuilding.get(Math.round(w.fx) + ',' + Math.round(w.fy)) || null; });
    if (reduced()) { paint(); pushState(); return; }
  ...
  function list() {
    return walkers.map(w => {
      ...
      return { id: w.id, name: w.name, color: w.color, inside };
    });
  }
