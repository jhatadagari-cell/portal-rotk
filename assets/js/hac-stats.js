...
const HacStats = (function () {
  ...
  // Bono % de PRESTIGIO ganado (ropa de torso rara). Fracción aditiva (0.05 = +5%).
  function bonusPrestigio(mid) { const r = row(mid); if (!r || !window.HacTienda) return 0; let s = 0; r.equipado.forEach(id => { const b = HacTienda.equipBonus(id); if (b && b.prestigioPct) s += b.prestigioPct; }); return s; }
  // Protección ANTIRROBO (fracción 0..0.8): reduce el dinero que te quitan al fracasar.
  function bonusAntirrobo(mid) { const r = row(mid); if (!r || !window.HacTienda) return 0; let s = 0; r.equipado.forEach(id => { const b = HacTienda.equipBonus(id); if (b && b.antirroboPct) s += b.antirroboPct; }); return Math.min(0.8, s); }
  // ── Efectos ÚNICOS de las ARMADURAS ÉPICAS (絕品) ─────────────────────────────
  // Suma genérica de un campo de `equip` sobre las piezas equipadas (fracción o entero).
  function bonusEquipCampo(mid, campo) { const r = row(mid); if (!r || !window.HacTienda) return 0; let s = 0; r.equipado.forEach(id => { const b = HacTienda.equipBonus(id); if (b && b[campo]) s += b[campo]; }); return s; }
  const bonusHeridaInmune = (mid) => Math.floor(bonusEquipCampo(mid, 'heridaInmune'));   // 戰神鎧: nº de heridas a ignorar
  const bonusInvestig = (mid) => bonusEquipCampo(mid, 'investigPct');                     // 鶴氅: +% aporte a investigación
  const bonusBotin = (mid) => bonusEquipCampo(mid, 'botinPct');                           // 江東鎧: +% probabilidad de botín
  const bonusAuraBanda = (mid) => bonusEquipCampo(mid, 'auraBanda');                       // 武聖袍: aura de éxito a la banda
